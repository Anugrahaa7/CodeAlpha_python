...
# Set eco-friendly background color
root.config(bg='#e0f7df')  # Light green background
...
# GUI Layout
word_label = tk.Label(root, textvariable=displayed_word, font=('Helvetica', 18), bg='#e0f7df')
word_label.pack(pady=20)

attempts_label = tk.Label(root, text="Attempts left: ", bg='#e0f7df')
attempts_label.pack()

attempts_left_label = tk.Label(root, textvariable=attempts_left, font=('Helvetica', 14), bg='#e0f7df')
attempts_left_label.pack()

incorrect_guesses_label = tk.Label(root, text="Incorrect guesses: ", font=('Helvetica', 12), bg='#e0f7df')
incorrect_guesses_label.pack(pady=10)

# Alphabet buttons for guessing letters
buttons_frame = tk.Frame(root, bg='#e0f7df')
buttons_frame.pack()

# Create buttons for each letter
for letter in 'abcdefghijklmnopqrstuvwxyz':
    btn = tk.Button(buttons_frame, text=letter.upper(), width=4, height=2,
                    command=lambda l=letter: guess_letter(l), bg="#8BC34A", fg="white")
    btn.grid(row=(ord(letter) - 97) // 9, column=(ord(letter) - 97) % 9)

# Reset button to start a new game
reset_button = tk.Button(root, text="Reset Game", command=reset_game, font=('Helvetica', 12), bg="#4CAF50", fg="white")
