...
root = tk.Tk()
root.title("Eco-friendly Chatbot")
...
chat_window.tag_configure("user_tag", foreground="white", background="#5cb85c", justify="left", font=("Helvetica", 12, "bold"), relief=tk.RAISED, lmargin1=10, lmargin2=10, rmargin=10)  # Green for user
chat_window.tag_configure("bot_tag", foreground="white", background="#4cae4c", justify="right", font=("Helvetica", 12, "italic"), relief=tk.RAISED, lmargin1=10, lmargin2=10, rmargin=10)  # Lighter green for bot

# Create a text entry box for typing user input
input_frame = tk.Frame(root, bg='#e0f7df')
input_box = tk.Entry(input_frame, font=("Helvetica", 14), width=28)
input_box.pack(side=tk.LEFT, pady=10, padx=10)

# Create a "Send" button
send_button = tk.Button(input_frame, text="Send", command=send_message, font=("Helvetica", 12), bg="#5cb85c", fg="white", width=8)
